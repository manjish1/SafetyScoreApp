...
# CONFIG
SERVICE_URL = "https://services3.arcgis.com/Et5Qfajgiyosiw4d/arcgis/rest/services/AxonCrimeData_Export_view/FeatureServer/19/query"
...
def filter_crimes_nearby(df, loc, radius_km=1.0):
    ...
    df["Distance_km"] = df.apply(
        lambda r: geodesic(loc, (r[latc], r[lonc])).km,
        axis=1
    )
...
def calculate_safety_score(df):
    ...
    for _, r in df.iterrows():
        # crime type
        ct = None
        for k in ("UCR_Literal","crime","CRIME"):
...
def index():
    ...
    return render_template(
        ...
        crimes=crimes,            # ← this matches the template
        address=address,
        error_message=error
    )
